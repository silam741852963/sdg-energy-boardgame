...
import math
import random
from .config import COLOR_MAP, COLORS, SCREEN_WIDTH, SCREEN_HEIGHT, FOV, VIEWER_DISTANCE, FIREWORK_TYPES
from . import palette
...
class ParticleSystem:
    ...
    def spawn(self, x, y, z, vx, vy, vz, spec, count=1, is_shell=False, is_inner=False, is_split_child=False, particle_color=None):
    ...
    def spawn_split_children(self, parent_indices):
        ...
        angles = np.tile([math.pi/4, 3*math.pi/4, 5*math.pi/4, 7*math.pi/4], num_parents)
    ...
    def update(self):
        ...
        if np.any(cull_eligible):
            ...
            cull_mask = (age_el > life_el) | (remaining < 0.12) | (intensities[cull_eligible] < 0.12)
        ...
        if np.any(trail_update_mask):
            self.history_x[trail_update_mask, :-1] = self.history_x[trail_update_mask, 1:]
            self.history_y[trail_update_mask, :-1] = self.history_y[trail_update_mask, 1:]
            self.history_factor[trail_update_mask, :-1] = self.history_factor[trail_update_mask, 1:]
        ...
        if np.any(waterfall_mask):
            self.gravity[waterfall_mask] = np.minimum(0.3, self.gravity[waterfall_mask] + 0.005)
        ...
        # 7. Split check
        split_eligible = active_mask & self.split & ~self.is_split_child & ~self.is_shell
    ...
    def gather_instances(self, frame_count):
        ...
        # 1. Main particles
        flicker_skip = self.has_flicker & (((frame_count + self.flicker_offset) % 6) < 3)
        crackle_phase2 = self.has_crackle & (self.age > self.life * 0.8)
        crackle_phase1 = self.has_crackle & (self.age > self.life * 0.5) & (self.age <= self.life * 0.8)
        ...
        if num_crackle > 0:
            ...
            px_crackle = self.px[crackle_indices] + np.random.randint(-3, 4, num_crackle)
            py_crackle = self.py[crackle_indices] + np.random.randint(-3, 4, num_crackle)
            ...
            crackle_chunk = np.column_stack((px_crackle, py_crackle, size_crackle, rgb_crackle, intensity_crackle))
        ...
        if np.any(has_trail_mask):
            ...
            for j in range(40):
                ...
                size_trail = np.maximum(3.0, hfactor * thickness * 12.0 * size_mult)
                ...
                if np.any(is_comet_pearl):
                    alpha_trail[is_comet_pearl] = np.minimum(1.0, alpha_trail[is_comet_pearl] * 1.5)
                    size_trail[is_comet_pearl] = size_trail[is_comet_pearl] * 1.8
                trail_chunk = np.column_stack((hx, hy, size_trail, rgb_trail, alpha_trail))
                ...
                if num_s2 > 0:
                    ...
                    sx2 = s2_hx + np.random.uniform(-scatter_radius - 2.0, scatter_radius + 2.0)
                    sy2 = s2_hy + np.random.uniform(-scatter_radius - 2.0, scatter_radius + 2.0)
                ...
                glitter_mask = self.glitter[sub_indices] & (np.random.rand(num_sub) < 0.4)
